...
export default function InventoryManagement() {
  ...
  fetch(`${API_URL}/categories/list`).then(r => r.json()).then(d => setCategories(d.categories || [])).catch(() => {});
  // fetch units list
  fetch(`${API_URL}/units/list`).then(r => r.json()).then(d => setUnits(d.units || [])).catch(() => {});
  }, []);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {showModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div className="absolute inset-0 bg-black bg-opacity-50 backdrop-blur-sm" onClick={() => setShowModal(false)} />
            <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl p-8 max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-6">
                <h3 className="text-2xl font-bold text-amber-900">{editId ? 'Edit Item' : 'Add New Item'}</h3>
                <button 
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {list.map(item => {
                        ...
                        return (
                        <tr key={item._id} className={`hover:bg-amber-50 transition-colors border-b border-amber-100 ${item.isExpired ? 'bg-red-50' : item.isLowStock ? 'bg-yellow-50' : ''}`}>
                          <td className="py-6 px-6">
                            {item.image ? (
                              <img src={`http://localhost:5000${item.image}`} alt={item.name} className="w-20 h-20 object-cover rounded-xl shadow-md border-2 border-white" />
                            ) : (
                          ...
                          <td className="py-6 px-6">
                            <div className="text-lg font-bold text-green-700">{formatCurrency(item.price)}</div>
                            {item.batchNumber && (
                          ...
                          <td className="py-6 px-6 text-sm">
                            {item.expirationDate ? (
                              <div>
                                <div className="font-medium text-amber-900">{new Date(item.expirationDate).toLocaleDateString()}</div>
                                {item.daysUntilExpiry !== null && (
                                  <div className={`text-xs mt-1 px-2 py-1 rounded ${item.daysUntilExpiry < 0 ? 'text-red-600 bg-red-50' : item.daysUntilExpiry <= 3 ? 'text-orange-600 bg-orange-50' : 'text-amber-500 bg-amber-50'}`}>
